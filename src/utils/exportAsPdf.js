...
import { generalCanvasByHtmlArray } from './exportAsImage'

const A4_WIDTH = 592.28
const A4_HEIGHT = 841.89

async function toCanvas(htmlStrings) {
  const tempCanvas = await generalCanvasByHtmlArray(htmlStrings)
  tempCanvas.data = tempCanvas.toDataURL('image/png')
  return tempCanvas
}
 ...
 * @param {number} [param.contentWidth] - 一页pdf的内容宽度，0-592.28
 * @param {string} [param.filename] - pdf文件名
 * @param {HTMLElement} param.header - 页眉dom元素
 * @param {HTMLElement} param.footer - 页脚dom元素
 */
export default async function exportAsPdf({ element, contentWidth = 550, footer, header, filename = '测试A4分页.pdf' }) {
  ...
  // 一页的高度， 转换宽度为一页元素的宽度
  const { width, height, data } = await toCanvas(element, { width: contentWidth })

  // 添加页脚
  async function addHeader(header, pdf, contentWidth) {
    const { height: headerHeight, data: headerData } = await toCanvas(header, { width: contentWidth })
  ...
  async function addFooter(pageNum, now, footer, pdf, contentWidth) {
    ...
    newFooter.querySelector('.pdf-footer-page').textContent = now
    newFooter.querySelector('.pdf-footer-page-count').textContent = pageNum
    document.documentElement.append(newFooter)
    const { height: footerHeight, data: footerData } = await toCanvas(newFooter, { width: contentWidth })
  ...
  // 页脚元素 经过转换后在PDF页面的高度
  const { height: tfooterHeight } = await toCanvas(footer, { width: contentWidth })

  // 页眉元素 经过转换后在PDF的高度
  const { height: theaderHeight } = await toCanvas(header, { width: contentWidth })
